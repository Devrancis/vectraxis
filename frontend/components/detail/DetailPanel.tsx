...
export default function DetailPanel({ technique, onClose }: DetailPanelProps) {
  ...
  // Add Escape key listener for power-user UX
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape" && technique) onClose();
    };
      ...
      {technique && (
        <>
          ...
          <motion.div
            ...
            className="absolute top-0 right-0 h-full w-[450px] bg-bg-surface border-l border-border shadow-[0_0_40px_rgba(0,0,0,0.8)] flex flex-col z-50 overflow-hidden"
          >
            {/* Header section */}
